...
// Inventory is household-wide — no ledger in the link, even if the row has one.
assert.equal(notificationUrl({ inventory_item_id: 7, ledger_id: LED }), "/?view=inventory");
...
assert.deepEqual(notificationTarget({ inventoryItemId: 7 }), { view: "inventory", ledgerId: null, expenseId: null });
...
assert.deepEqual(roundTrip({ inventory_item_id: 7 }), { view: "inventory", ledgerId: null, expenseId: null });
