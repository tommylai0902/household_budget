...
// across every ledger, past RLS. The upsert shape and the cycle_date guard are
// deliberately identical; if one changes the other has to follow.
...
export default async function handler(req, res) {
  ...
  try {
    // ---- 1. generate expiry reminders across every ledger ----
    ...
    // Fetched once — both halves of this job need it (titles below, recipients
    // further down).
    ...
    // which is shared by everyone on the ledger, so there is no single right
    ...
    const langOf = (ledgerId) =>
      [...(usersByLedger.get(ledgerId) || [])].map((u) => langByUser.get(u)).find(Boolean) || "en";

    let created = 0;
    for (const item of items || []) {
      if (trackedByItem.get(item.id) === item.expiry_date) continue; // already built for this date
      const lang = langOf(item.ledger_id);
    ...
    for (const n of due) {
      let anySent = false;
      for (const userId of usersByLedger.get(n.ledger_id) || []) {
